...
//  NAIST Open Campus
//
//  Created by Edess Akpa on 10/21/16.
...
final class LightMQTT: NSObject, StreamDelegate {
    ...
    fileprivate func connectSocket(_ host: String, port: Int) -> Bool {
        ...
        inputStream?.scheduleInRunLoop(.mainRunLoop(), forMode: RunLoopMode.defaultRunLoopMode)
        outputStream?.scheduleInRunLoop(.mainRunLoop(), forMode: RunLoopMode.defaultRunLoopMode)
        
        return true
    }
    
    fileprivate func disconnectSocket() {
        inputStream?.removeFromRunLoop(.mainRunLoop(), forMode: RunLoopMode.defaultRunLoopMode)
        outputStream?.removeFromRunLoop(.mainRunLoop(), forMode: RunLoopMode.defaultRunLoopMode)
    ...
    dynamic internal func stream(_ stream: Stream, handle eventCode: Stream.Event) {
        ...
        case inputStream!:
            ...
            case StreamEvent.OpenCompleted:
                clientState = MQTTClientState.decodingHeader
            case StreamEvent.ErrorOccurred:
                clientState = MQTTClientState.connectionError
            case StreamEvent.EndEncountered:
                clientState = MQTTClientState.connectionClosed
            case StreamEvent.None:
                break
            default:
                break
            }
